tdir=hiveext-test-`date +"%Y.%m.%d-%H.%M"`
mkdir $tdir
...
for p in `seq -w 1 $1`
do
    export CONTAINER_NAME=hiveext$p
...
echo "hiveext-parallel with $1 containers started at `date` using ${HOST_OUTPUT_STYLE} logging to $tdir" | tee > summary.txt
